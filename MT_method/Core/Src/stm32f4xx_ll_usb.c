...
#include <stdint.h> // added by Prof. Lee
#include "stm32f4xx_hal_def.h"  // added by Prof. Lee
#include "stm32f4xx_ll_usb.h"   // added by Prof. Lee
void DWT_ms_Delay(uint32_t);    // added by Prof. Lee. HAL_Delay 대신 사용하기 위해 필요
...
HAL_StatusTypeDef USB_SetCurrentMode(USB_OTG_GlobalTypeDef *USBx , USB_OTG_ModeTypeDef mode)
{
  ...
  //HAL_Delay(50U);  // deleted by Prof. Lee. HAL_Delay는 SysTick을 사용해야 하지만 우리는 DWT를 이용할 것임.
  DWT_ms_Delay(50U); // added by Prof. Lee
...
HAL_StatusTypeDef  USB_DevConnect (USB_OTG_GlobalTypeDef *USBx)
{
  ...
  //HAL_Delay(3U);   // deleted by Prof. Lee
  DWT_ms_Delay(3U);  // added by Prof. Lee
